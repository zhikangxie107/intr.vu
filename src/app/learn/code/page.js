  ...
  export default function InterviewPage() {
    ...
    const handleTranscript = async (text) => {
      ...
      try {
        ...
        var promptToAppend = {role: 'user', content: prompt};
        var responseToAppend = {role: 'interviwer', content: textAnswer};
    ...
    // use effect to load problem data
    useEffect(() => {
      if (!name) return;
    ...
    // use effect to create session after problem loads
    useEffect(() => {
    ...
    // first time sessionID is set make interviwer do a greeting prompt
    useEffect(() => {
      if (!sessionID) return;
      const doGreeting = async () => {
        const greetingPrompt = "Greet the candidate and ask them to describe their approach to solving the problem so far.";
    ...
    // use effect to call rich  ask every 45 seconds with asking me a question
    // get session every time, if it is active, ask a question
    useEffect(() => {
      if (!sessionID) return;
